...
const VoteSummary = ({
  ...
  const votesYeaPct = safeDiv(votesYea, totalVotes)
  const votesNayPct = safeDiv(votesNay, totalVotes)
          ...
          <Main>
            ...
            <BarWrapper>
              <QuorumBar
                style={{
                  transform: `
                  translateX(${PANEL_INNER_WIDTH * 50 * progress}px)
                  scaleY(${progress})
                `,
                }}
              />
            ...
            <YesNoItem color={theme.positive}>
              <span>Yes</span>
              <strong>{Math.round(votesYeaPct * progress * 100)}%</strong>
            </YesNoItem>
            <YesNoItem color={theme.negative}>
              <span>No</span>
              <strong>{Math.round(votesNayPct * progress * 100)}%</strong>
            </YesNoItem>
          </Main>
        )
      }}
